...
import { moderatorRequestsService } from "../services/api";
...
export default function ModeratorRequests() {
  ...
  const handleReviewClick = (request: ModeratorRequest, action: 'APPROVED' | 'REJECTED') => {
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      await moderatorRequestsService.review(selectedRequest.request_id, reviewAction);
      toast({
        title: "Sucesso!",
      ...
      toast({
        title: "Erro",
        description: error.response?.data?.message || "Não foi possível processar a solicitação",
        variant: "destructive",
      });
    } finally {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'APPROVED':
        return <Badge className="bg-green-600 hover:bg-green-700"><CheckCircle className="w-3 h-3 mr-1" /> Aprovada</Badge>;
      case 'REJECTED':
        return <Badge className="bg-red-600 hover:bg-red-700"><XCircle className="w-3 h-3 mr-1" /> Rejeitada</Badge>;
      default:
    ...
    <div className="min-h-screen bg-[#2B71A3]">
        ...
        <div className="bg-[#274584] rounded-t-[25px] min-h-[861px] pt-[103px] px-4 sm:px-8 lg:px-[143px]">
          ...
          <Tabs defaultValue="pending" className="w-full">
            <TabsList className="bg-[#1F2937] border-gray-700">
              ...
              <TabsTrigger value="reviewed" className="data-[state=active]:bg-blue-600">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {pendingRequests.map((request) => (
                        <TableRow key={request.request_id} className="border-gray-700 hover:bg-[#374151]">
                          ...
                          <TableCell className="text-gray-300">{request.user.user_email}</TableCell>
            ...
            <TabsContent value="reviewed" className="mt-6">
              {reviewedRequests.length === 0 ? (
                <div className="bg-[#1F2937] border border-gray-700 rounded-lg p-12 text-center">
                  <p className="text-gray-400 text-lg">Nenhuma solicitação revisada ainda</p>
                </div>
              ) : (
                <div className="bg-[#1F2937] border border-gray-700 rounded-lg overflow-hidden">
                  <Table>
                    <TableHeader>
                      <TableRow className="border-gray-700 hover:bg-[#1F2937]">
                        ...
                        <TableHead className="text-gray-300">Revisado por</TableHead>
                        <TableHead className="text-gray-300">Data da Revisão</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {reviewedRequests.map((request) => (
                        <TableRow key={request.request_id} className="border-gray-700 hover:bg-[#374151]">
                          ...
                          <TableCell className="text-gray-300">
                            {request.reviewer ? request.reviewer.full_name : 'N/A'}
                          </TableCell>
                          <TableCell className="text-gray-300">
      ...
      <Dialog open={isReviewDialogOpen} onOpenChange={setIsReviewDialogOpen}>
        <DialogContent className="bg-[#1F2937] border-gray-700 text-white">
          <DialogHeader>
            <DialogTitle>
              {reviewAction === 'APPROVED' ? 'Aprovar Solicitação' : 'Rejeitar Solicitação'}
            </DialogTitle>
          ...
          <DialogFooter>
            ...
            >
              {isSubmitting ? "Processando..." : reviewAction === 'APPROVED' ? 'Aprovar' : 'Rejeitar'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
